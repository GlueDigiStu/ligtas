<?php 
    get_header(); 
    $category = get_the_category($post->ID); 
    $term_id = $category[0]->cat_ID;
    if($category[0]->parent) {
        $term_parent = $category[0]->parent;
    } else {
        $term_parent = $term_id;
    }
    $tf = 'category_'.$term_id;
    $tfp = 'category_'.$term_parent;
?>

        <div class="top_article top_page bg_grey">
            <div class="container">
                <div class="top_page_inner">
                	<?php lsx_breadcrumbs(); ?>
                    <div class="top_page_row">
                        <h1 class="title_section fade_in"><?php if(get_field('title')) { echo get_field('title'); } else { the_title(); } ?></h1>
                        <div class="top_details_post">
                            <p><?php if(get_field('short_title',$tf)) { echo get_field('short_title',$tf); } else { echo get_cat_name($term_id); } ?></p>
                            <p><?php the_time("F Y"); ?></p>
                        </div>
                    </div>
                </div>
            </div>
        </div>

        <?php 
            $topimg = '';
            if(get_field('top_image')==1 && has_post_thumbnail()){ 
            	$thumb = wp_get_attachment_image_src(get_post_thumbnail_id(),'full',true);
            	$topimg = $thumb[0];
            } elseif(get_field('top_image')==2) {
            	$img = get_field('image'); 
            	$topimg = $img['url'];
            }
            if($topimg) {
        ?>
        <div class="top_img bg_grey_half">
            <div class="container">
                <div class="top_img_inner fade_in">
                    <img src="<?php echo esc_url($topimg); ?>" alt="<?php the_title(); ?>">
                </div>
            </div>
		</div>
        <?php } ?>

		<?php while (have_posts()) : the_post(); ?>
		<?php the_content(); ?>
		<?php endwhile; ?>

        <?php 
            $arg = array('cat'=>$term_parent, 'post__not_in'=>array($post->ID), 'post_type'=>'post', 'posts_per_page'=>2);
            $Nquery = new WP_Query($arg); if($Nquery->have_posts()):
        ?>
        <div class="news_section content_section p_160 bg_grey">
			<div class="container">
				<div class="panel_content">
					<div class="panel_content_left fade_left">
						<div class="panel_content_description">
							<h3 class="title_category"><?php echo get_cat_name($term_parent); ?></h3>
							<?php if(get_field('title_more_posts',$tfp)) { ?>
							<h2 class="title_section"><?php echo get_field('title_more_posts',$tfp); ?></h2>
							<?php } ?>
						</div>
					</div>
					<div class="panel_content_right fade_right">
						<a class="btn" href="<?php echo get_category_link($term_parent); ?>">
							View All
							<svg class="svg_arrow_btn">
								<use xlink:href="<?php bloginfo('template_url'); ?>/images/sprite/sprite.svg#arrow_btn"></use>
							</svg>
						</a>
					</div>				
				</div>
				<div class="posts_slider swiper fade_in">
					<div class="swiper-wrapper">
                        <?php $i = 0; while($Nquery->have_posts()): $Nquery->the_post(); $i++; $category = get_the_category($post->ID); ?>
						<div class="post_slide <?php if($i%2===0) { echo 'even'; } else { echo 'odd'; } ?> swiper-slide">
							<a class="post_item" href="<?php the_permalink(); ?>">
								<div class="post_item_top">
									<div class="post_item_details">
										<p><?php if(get_field('short_title','category_'.$category[0]->cat_ID)) { echo get_field('short_title','category_'.$category[0]->cat_ID); } else { echo get_cat_name($term_id); } ?></p>
                                        <!--<p class="tags"><?php 
                                            $tags = get_the_tags();
                                            if($tags && !is_wp_error($tags)) {
                                                echo esc_html( join(', ', wp_list_pluck($tags, 'name')) );
                                            }
                                        ?></p>-->										
                                        <p><?php the_time("F Y"); ?></p>
									</div>
									<?php if(has_post_thumbnail()){ $thumb = wp_get_attachment_image_src(get_post_thumbnail_id(),'full',true); ?>
									<div class="post_item_img">
										<img src="<?php echo esc_url($thumb[0]); ?>" alt="<?php the_title(); ?>">
									</div>
									<?php } ?>
								</div>
								<div class="post_item_info">
									<p class="post_item_name"><?php the_title(); ?></p>
									<div class="post_item_more">
										<span class="post_item_link">Learn More</span>
										<span class="post_item_arrow">
											<svg class="svg_arrow_post">
												<use xlink:href="<?php bloginfo('template_url'); ?>/images/sprite/sprite.svg#arrow_post"></use>
											</svg>
										</span>
									</div>
								</div>
							</a>
						</div>
					    <?php endwhile; ?>
					</div>
					<div class="posts_nav">
						<div class="posts_arrow posts_prev">
							<svg class="svg_arrow_prev">
								<use xlink:href="<?php bloginfo('template_url'); ?>/images/sprite/sprite.svg#arrow_prev"></use>
							</svg>
						</div>
						<div class="posts_arrow posts_next">
							<svg class="svg_arrow_next">
								<use xlink:href="<?php bloginfo('template_url'); ?>/images/sprite/sprite.svg#arrow_next"></use>
							</svg>
						</div>
					</div>
				</div>
			</div>
		</div>
	    <?php endif; wp_reset_query(); ?>

        <?php 
            if(have_rows('sections',$tfp)): while(have_rows('sections',$tfp)): the_row(); 
               $sections = get_post(get_sub_field('section'));
               $blocks = parse_blocks( $sections->post_content );
               echo render_block($blocks[0]);  
            endwhile; endif; 
        ?>
	
<?php get_footer(); ?>