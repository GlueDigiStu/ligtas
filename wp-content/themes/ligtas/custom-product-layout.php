            ...
            <div class="course_intro_wrap">
                <div class="course_intro_left">
                    <div class="course_intro_info">
                        ...
                        <?php if (is_array($header_button) && !empty($header_button['url'])): ?>
                            <a class="btn fade_in"
                               href="<?= $header_button['url'] ?>"><?= $header_button['title'] ?></a>
                        <?php else: ?>
                            <a class="btn fade_in"
                ...
                <div class="course_intro_right fade_right">
                    ...
                    if ($course_overview['classroom_courses']['enable'])
                        $count++;
                    if ($course_overview['in_company_courses']['enable'])
                        $count++;
                    ?>
                    <div class="x:md:absolute x:bottom-2 x:left-2 x:right-2 x:grid x:md:grid-cols-<?= $count ?> x:gap-4">
                        ...
                        <?php if ($course_overview['in_company_courses']['enable']): ?>
                            <div class="x:bg-[#291261] x:rounded-[10px] x:p-4">
...
<?php
$enable_online_courses = get_field('enable_online_courses');

if ($enable_online_courses):
    ?>
    <style>
                            ...
                            <table class=" x:w-full x:mx-auto x:mt-20 details_table x:[&_a]:text-[#a27fff]">
                                ...
                                <tr>
                                    ...
                                    <td style="text-align: center;"
                                        class="x:text-center x:text-2xl x:font-bold x:relative">
...
$enable_virtual_courses = get_field('enable_virtual_courses');
    ...
    <div id="virtual-courses" style="padding-top: 100px; margin-top: -100px;" class="container x:pt-8 x:mt-12">
        ...
        <?php if ($cta = get_field('virtual_courses_cta') || get_field('virtual_courses_cta_button')): ?>
            <div class="x:flex x:flex-wrap x:justify-start  x:items-center x:gap-4 x:pb-20">
                <div class="typical_text">
                    <?php $cta = get_field('virtual_courses_cta') ?>
                    <?= $cta ?>
                </div>
...
$enable_in_company_courses = get_field('enable_in_company_courses');

if ($enable_in_company_courses):?>
    <div id="in-company-courses" style="padding-top: 100px; margin-top: -100px;">
    <div class="x:bg-[#ffffff]">
    <div class="container">
        <div class="x:pt-20 x:pb-8">
            <div class="typical_text">
                <h3 class="title_block x:flex x:items-center x:gap-2 x:mb-12 x:text-[#291261]">
                    ...
                    <?php $in_company_courses_title = get_field('in_company_courses_title'); ?>
                    <?= $in_company_courses_title ?: 'Our In Company Based Course Options' ?>
                </h3>

                <?php if ($in_company_courses_helper_text = get_field('in_company_courses_helper_text')) { ?>
                    <div class="x:pb-12"><?= $in_company_courses_helper_text ?></div>
                <?php } ?>
            </div>

            <?php if (get_field('in_company_courses_cta') || get_field('in_company_courses_cta_button')): ?>
                <div class="">
                    <div class="x:flex x:flex-wrap x:justify-start  x:items-center <?php if (get_field('in_company_courses_cta')): ?> x:gap-4 <?php endif; ?>  x:pb-20">
                        <div class="typical_text">
                            <?= get_field('in_company_courses_cta') ?>
                        </div>
                        <?php if ($btn = get_field('in_company_courses_cta_button')): ?>
                            <div class="x:md:basis-1/6">
                                <?php if (is_array($btn)): ?>
                                    <div class="x:ml-auto x:flex">
                                        <a class="btn  x:text-center x:inline-block"
